...
public class NetworkServiceImpl extends NetworkServiceBase implements NetworkService {
    ...
    public NetworkServiceImpl(NetworkConfiguration networkConfiguration) {
        ...
        this.executor = ThreadUtils.getSingleThreadExecutorService("headerSV-networkService");
    }
    ...
    private void monitor() {
        try {
            while(true) {
                //if the service was paused due to min peers not being reached, check if they've since been connected
                if (!serviceStarted) {
                    if (checkMinimumPeersConnected()) {
                        onEvent(new MinimumPeersReachedEvent());
                    }
                }
                Thread.sleep(1000);
            }
